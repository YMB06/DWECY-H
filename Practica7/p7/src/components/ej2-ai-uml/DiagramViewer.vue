...
// Usar desestructuración del store para que sea reactive
  ...
  async (newCode: string) => {
    if (newCode && mermaidContainer.value) {
      try {
        ...
        const result: any = await mermaid.render('mermaid-diagram', newCode as string)
        const svgContent: string = typeof result === 'string' ? result : result.svg
        mermaidContainer.value.innerHTML = svgContent
      } catch (error) {
